...
def write_data(data, export_format):
    '''
    Dispatches call to a specific transformer and returns data setself.
...
def write_csv(data):
    ...
    '''
    # Using this to simulate file IO, as csv can only write to files.
    f = StringIO.StringIO()
    writer = csv.writer(f)
    for row in data:
        writer.writerow(row)
...
def write_xlsx(data):
    ...
    for line in data:
        ...
        for datum in line:
            ...
            col += 1
        row += 1
        # We have hard limit here of 65535 rows that we are able to
        # save in spreadsheet.
        if row > 65535:
            print("Hit limit of # of rows in one sheet (65535).", file=sys.stderr)
            break
    # XLS is special case where we have to save the file and just return 0
    f = StringIO.StringIO()
    book.save(f)
    return f.getvalue()
...
if __name__ == '__main__':
    ...
    parse.add_argument("import_file", help="Path to a fixed-width data file.")
    ...
    if args.export_format not in ('csv', 'json', 'xlsx'):
        print("You must provide valid export file format.")
        sys.exit(1)

    # Verify given path is accessible file
    if not os.path.isfile(args.import_file):
        print("Given path is not a file: {:s}".format(args.import_file))
        sys.exit(1)

    # read from formatted fixed-width file
    data = import_data(args.import_file)

    # export data to specified format to make thsi Unix-like pipe-able
